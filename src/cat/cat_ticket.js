...
const infoContent = files.read('./info_ticket.json');
...
ui.reset.on('click', function(){
    ...
    resetDialog.show();
    function Upgrade(){
        jsUrl = 'https://cauto.oss-rg-china-mainland.aliyuncs.com/cat/cat_ticket.js'
        infoUrl = 'https://cauto.oss-rg-china-mainland.aliyuncs.com/info/info_ticket.json'
        toast('正在下载更新资源')
        http.get(jsUrl, {}, function(res, err){
            ...
            if(res.statusCode === 200){
                ...
                var savePath = engines.myEngine().cwd() + '/cat_ticket.js';
        ...
        http.get(infoUrl, {}, function(res, err){
            ...
            if(res.statusCode === 200){
                let scriptContent = res.body.string();
                var savePath = engines.myEngine().cwd() + '/info_ticket.json';
                files.write(savePath, scriptContent);
                engines.execScriptFile(savePath);
                toast('更新说明成功');
            } else {
...
ui.list.on('item_bind', (itemView, itemHolder) => {
    ...
    itemView.download.on("click", function(){/* 下载功能 */
        ...
        var url = 'https://cauto.oss-rg-china-mainland.aliyuncs.com/scripts/' + scriptName;
