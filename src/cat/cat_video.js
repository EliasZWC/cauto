...
const infoContent = files.read('./info_video.json');
...
ui.reset.on('click', function(){
    ...
    resetDialog.show();
    function Upgrade(){
        jsUrl = 'https://cauto.oss-rg-china-mainland.aliyuncs.com/cat/cat_video.js'
        infoUrl = 'https://cauto.oss-rg-china-mainland.aliyuncs.com/info/info_video.json'
        toast('正在下载更新资源')
        http.get(jsUrl, {}, function(res, err){
            ...
            if(res.statusCode === 200){
                ...
                var savePath = engines.myEngine().cwd() + '/cat_video.js';
        ...
        http.get(infoUrl, {}, function(res, err){
            ...
            if(res.statusCode === 200){
                let scriptContent = res.body.string();
                var savePath = engines.myEngine().cwd() + '/info_video.json';
                files.write(savePath, scriptContent);
                engines.execScriptFile(savePath);
                toast('更新说明成功');
            } else {
